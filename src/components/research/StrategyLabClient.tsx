...
type CopyBriefStatus = "idle" | "copied" | "manual";
...
function buildResearchBrief(snapshot: StrategyLabSnapshot): string {
  const resolvedPairCount = snapshot.simulation?.stats.pairCount
    ?? snapshot.calibration?.pairCount
    ?? snapshot.predictions.recentResolved.length;
  const validPairCount = snapshot.simulation?.stats.validPairCount
  ...
  return [
    ...
    "- Symbol diagnostics such as 0050 or 2317, when present, are descriptive artifact diagnostics only, not recommendations.",
  ].join("\n");
}
...
export function StrategyLabClient({ initialSnapshot }: StrategyLabClientProps) {
  ...
  const [copyBriefStatus, setCopyBriefStatus] = useState<CopyBriefStatus>("idle");
  const style = decisionStyle(snapshot.productStance.decision);
  const executiveSummary = buildExecutiveSummary(snapshot);
  const researchBrief = buildResearchBrief(snapshot);
...
function ResearchBriefSection({
  ...
  const statusText = copyStatus === "copied"
    ? "已複製到剪貼簿。"
    : copyStatus === "manual"
      ? "剪貼簿不可用，請直接選取下方文字手動複製。"
      : "可直接複製，或手動選取下方文字。";
  ...
  return (
    <section className="min-w-0 rounded-lg border border-border/50 bg-card/70 p-5">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="mb-1 flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">複製研究摘要</h2>
          </div>
          <p className="max-w-4xl text-sm leading-6 text-muted-foreground">
            由目前頁面快照產生的保守 Markdown 摘要；不下載、不發布、不呼叫外部服務。
          </p>
        </div>
        <button
          type="button"
          onClick={onCopy}
          className="inline-flex h-10 shrink-0 items-center gap-2 rounded-md border border-white/15 bg-white/8 px-3 text-sm font-medium text-white transition-colors hover:bg-white/14"
        >
      ...
      <label className="sr-only" htmlFor="strategy-lab-research-brief">
        Strategy Lab Research Brief
      </label>
      <textarea
        id="strategy-lab-research-brief"
        readOnly
        value={brief}
        className="min-h-[300px] w-full resize-y rounded-md border border-border/40 bg-background/60 p-3 font-mono text-xs leading-5 text-foreground outline-none"
      />
