...
**Problem 1 — system_health.** Most recent StockQuote was 376h ago. Market data sync may be broken or incomplete, silently degrading all downstream signal quality.

Evidence: `Latest StockQuote: 2026-04-18T16:28:29.000Z`; `Threshold: must be within 2 trading days`; `Affected stock: 2014`

**Problem 2 — test_coverage.** 42 orchestrator modules have no test file: adaptivePolicy.test, aiModulesService.test, aiService.test, llmUsageWarnings, providerCapabilities. Critical planner/worker/gate logic runs without regression protection.

Evidence: `src/lib/agent-orchestrator/adaptivePolicy.test.ts — no test file`; `src/lib/agent-orchestrator/aiModulesService.test.ts — no test file`; `src/lib/agent-orchestrator/aiService.test.ts — no test file`
...
- `src/lib/agent-orchestrator/llmUsageWarnings.ts`
- `src/lib/agent-orchestrator/providerCapabilities.ts`
...
- [ ] [system] Test files created for: adaptivePolicy.test, aiModulesService.test, aiService.test, llmUsageWarnings
...
**Problem 1 — ui_ux.** 17 page directories lack loading.tsx or error.tsx. Operators see blank screens or crashes during slow fetches or API errors.

Evidence: `Missing loading/error in src/app/asset-doubling`; `Missing loading/error in src/app/calendar`; `Missing loading/error in src/app/candidates`

**Problem 2 — wiki_docs.** USER_GUIDE.md was last modified 108 days ago. Recent orchestrator, CTO review, and daemon autostart changes are undocumented.

Evidence: `USER_GUIDE.md mtime: 2026-01-16T08:20:18.352Z`; `Age: 108 days since last update`
...
**Problem 1 — price_analysis_quality.** Price data quality issues detected: Latest quote: 2026-04-18T16:28:29.000Z (stale > 48h); 38 StockQuote rows with volume ≤ 0.

Evidence: `Latest quote: 2026-04-18T16:28:29.000Z (stale > 48h)`; `38 StockQuote rows with volume ≤ 0`
