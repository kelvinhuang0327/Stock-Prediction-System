...
**Problem 1 — system_health.** Most recent StockQuote was 150h ago. Market data sync may be broken or incomplete, silently degrading all downstream signal quality.

Evidence: `Latest StockQuote: 2026-04-18T16:28:29.000Z`; `Threshold: must be within 2 trading days`; `Affected stock: 2014`

**Problem 2 — test_coverage.** 20 orchestrator modules have no test file: autoCommit, plannerTick, profile, providers, service. Critical planner/worker/gate logic runs without regression protection.

Evidence: `src/lib/agent-orchestrator/autoCommit.ts — no test file`; `src/lib/agent-orchestrator/plannerTick.ts — no test file`; `src/lib/agent-orchestrator/profile.ts — no test file`

**Suggested Files:**
- `scripts/`
- `src/lib/sync/`
- `docs/DATA_SYNC_GUIDE.md`
- `src/lib/agent-orchestrator/autoCommit.ts`
...
- [ ] [system] Test files created for: autoCommit, plannerTick, profile, providers
...
Evidence: `Missing loading/error in src/app`; `Missing loading/error in src/app/asset-doubling`; `Missing loading/error in src/app/analysis`

**Problem 2 — wiki_docs.** USER_GUIDE.md was last modified 99 days ago. Recent orchestrator, CTO review, and daemon autostart changes are undocumented.

Evidence: `USER_GUIDE.md mtime: 2026-01-16T08:20:18.352Z`; `Age: 99 days since last update`
