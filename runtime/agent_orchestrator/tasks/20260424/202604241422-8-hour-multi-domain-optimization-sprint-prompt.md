...
**Problem 1 — system_health.** Most recent StockQuote was 142h ago. Market data sync may be broken or incomplete, silently degrading all downstream signal quality.

Evidence: `Latest StockQuote: 2026-04-18T16:28:29.000Z`; `Threshold: must be within 2 trading days`; `Affected stock: 2014`

**Problem 2 — test_coverage.** 19 orchestrator modules have no test file: adaptivePolicy, backlogService, classifierCalibration, common, ctoReviewTick. Critical planner/worker/gate logic runs without regression protection.

Evidence: `src/lib/agent-orchestrator/adaptivePolicy.ts — no test file`; `src/lib/agent-orchestrator/backlogService.ts — no test file`; `src/lib/agent-orchestrator/classifierCalibration.ts — no test file`
...
- [ ] [system] Test files created for: adaptivePolicy, backlogService, classifierCalibration, common
