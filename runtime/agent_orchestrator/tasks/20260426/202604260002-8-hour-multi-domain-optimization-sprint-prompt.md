...
**Problem 1 — system_health.** Most recent StockQuote was 176h ago. Market data sync may be broken or incomplete, silently degrading all downstream signal quality.

Evidence: `Latest StockQuote: 2026-04-18T16:28:29.000Z`; `Threshold: must be within 2 trading days`; `Affected stock: 2014`

**Problem 2 — test_coverage.** 20 orchestrator modules have no test file: service, signalStateClassifier, storage, tasks, types. Critical planner/worker/gate logic runs without regression protection.

Evidence: `src/lib/agent-orchestrator/service.ts — no test file`; `src/lib/agent-orchestrator/signalStateClassifier.ts — no test file`; `src/lib/agent-orchestrator/storage.ts — no test file`
...
- [ ] [system] Test files created for: service, signalStateClassifier, storage, tasks
...
**Problem 2 — wiki_docs.** USER_GUIDE.md was last modified 100 days ago. Recent orchestrator, CTO review, and daemon autostart changes are undocumented.

Evidence: `USER_GUIDE.md mtime: 2026-01-16T08:20:18.352Z`; `Age: 100 days since last update`
