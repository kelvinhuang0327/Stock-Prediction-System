...
**Problem 1 — system_health.** Most recent StockQuote was 311h ago. Market data sync may be broken or incomplete, silently degrading all downstream signal quality.

Evidence: `Latest StockQuote: 2026-04-18T16:28:29.000Z`; `Threshold: must be within 2 trading days`; `Affected stock: 2014`

**Problem 2 — test_coverage.** 30 orchestrator modules have no test file: schedulerGuard, types, adaptivePolicy.test, aiModulesService.test, aiService.test. Critical planner/worker/gate logic runs without regression protection.

Evidence: `src/lib/agent-orchestrator/schedulerGuard.ts — no test file`; `src/lib/agent-orchestrator/types.ts — no test file`; `src/lib/agent-orchestrator/adaptivePolicy.test.ts — no test file`
...
- `src/lib/agent-orchestrator/autoCommit.test.ts`
...
- [ ] [system] Test files created for: schedulerGuard, types, adaptivePolicy.test, aiModulesService.test
...
Evidence: `Missing loading/error in src/app`; `Missing loading/error in src/app/analysis`; `Missing loading/error in src/app/backtest`

**Problem 2 — wiki_docs.** USER_GUIDE.md was last modified 105 days ago. Recent orchestrator, CTO review, and daemon autostart changes are undocumented.

Evidence: `USER_GUIDE.md mtime: 2026-01-16T08:20:18.352Z`; `Age: 105 days since last update`
...
**Problem 1 — price_analysis_quality.** Price data quality issues detected: Latest quote: 2026-04-18T16:28:29.000Z (stale > 48h); 38 StockQuote rows with volume ≤ 0; 1 active simulated-trade symbols need fresh price data.

Evidence: `Latest quote: 2026-04-18T16:28:29.000Z (stale > 48h)`; `38 StockQuote rows with volume ≤ 0`; `1 active simulated-trade symbols need fresh price data`
