...
**Problem 1 — system_health.** Most recent StockQuote was 310h ago. Market data sync may be broken or incomplete, silently degrading all downstream signal quality.

Evidence: `Latest StockQuote: 2026-04-18T16:28:29.000Z`; `Threshold: must be within 2 trading days`; `Affected stock: 2014`

**Problem 2 — test_coverage.** 30 orchestrator modules have no test file: aiModulesService, aiService, llmExecutionPolicy, llmUsageLogger, schedulerGuard. Critical planner/worker/gate logic runs without regression protection.

Evidence: `src/lib/agent-orchestrator/aiModulesService.ts — no test file`; `src/lib/agent-orchestrator/aiService.ts — no test file`; `src/lib/agent-orchestrator/llmExecutionPolicy.ts — no test file`

**Suggested Files:**
- `scripts/`
- `src/lib/sync/`
- `docs/DATA_SYNC_GUIDE.md`
- `src/lib/agent-orchestrator/aiModulesService.ts`
- `src/lib/agent-orchestrator/aiService.ts`
- `src/lib/agent-orchestrator/llmExecutionPolicy.ts`
- `src/lib/agent-orchestrator/llmUsageLogger.ts`
...
- [ ] [system] Test files created for: aiModulesService, aiService, llmExecutionPolicy, llmUsageLogger
...
**Problem 1 — price_analysis_quality.** Price data quality issues detected: Latest quote: 2026-04-18T16:28:29.000Z (stale > 48h); 38 StockQuote rows with volume ≤ 0; 2 active simulated-trade symbols need fresh price data.

Evidence: `Latest quote: 2026-04-18T16:28:29.000Z (stale > 48h)`; `38 StockQuote rows with volume ≤ 0`; `2 active simulated-trade symbols need fresh price data`
